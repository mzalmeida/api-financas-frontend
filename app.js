...
function renderCardBillSummary() {
  ...
  summary.cards.forEach((row) => {
    ...
    if (row.installment_summary?.length) card.appendChild(createNode("span", "mini-copy", row.installment_summary.map((item) => `Parcela ${item.installment_number} - ${formatCurrency(item.amount)}`).join(" • ")));
